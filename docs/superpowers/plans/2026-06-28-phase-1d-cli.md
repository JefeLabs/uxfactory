...
**Tech Stack:** TypeScript 6.0.3 (ESM, NodeNext), `commander@14.0.1`, the global `fetch` (Node 20+), `node:fs/promises` for the atomic queue writer, Vitest 4.1.9 driving the command actions against a **real in-process `@uxfactory/bridge`** (started on port 0 with a temp `dataDir`). Same monorepo toolchain as Phase 0/1a/1b/1c.
...
- Runtime deps: `commander@14.0.1`, `@uxfactory/spec: workspace:*` (for `validate`), `@uxfactory/bridge: workspace:*` (for `startBridge`). devDep `@types/node@26.0.1`. The CLI uses the global `fetch` (Node 20+) for HTTP — no http-client dep.
...
- Constraints: `commander@14.0.1`, `@uxfactory/spec`/`@uxfactory/bridge` workspace deps, `@types/node@26.0.1` devDep, no http-client dep (global `fetch`) → Task 1 package.json. `paths` only in `tsconfig.typecheck.json`; build `tsconfig.json` rootDir "." / outDir "dist"; `exports`→`dist/src/index.js`, `bin`→`dist/src/cli.js`, `files:["dist"]` → Task 1. Root vitest `@uxfactory/bridge` alias → Task 1. Built artifact + whole-monorepo green → Task 6 Steps 8-9. ✅
