...
- **Badge placement / visual overlap.** Badges are currently positioned at `(target.x + target.width, target.y)` (top-right corner). On a real canvas this may overlap other content or fall outside the viewport. Live behaviour to decide: offset with a constant padding, use `target.absoluteTransform` to account for parent transforms, or position relative to the node's bounding box using `absoluteBoundingBox`.
...
- **Text node font availability.** `loadFontAsync({ family: "Inter", style: "Regular" })` is called before setting `notes-content` text. On some Figma teams that don't have Inter loaded, this may throw. A production version should catch font-load errors and fall back to the default document font or skip the text node with a note in the report.
