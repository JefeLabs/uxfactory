...
12. ~~Coverage semantics~~ **Resolved (2026-07-06, direction)**: the collision is naming, not semantics — the dial (generation breadth) and the metric (conformed features / total) measure different things and coexist. The dial renames (e.g. "Breadth") when the Coverage metric ships in Checks with `features` as its denominator. Rename deferred until then to avoid two churns.
