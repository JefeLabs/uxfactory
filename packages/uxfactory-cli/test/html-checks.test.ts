...
describe("runHtmlBatch", () => {
  ...
  it("runs all checks at visual:medium and is clean when all binding checks pass", () => {
    ...
    expect(r.checks.map((c) => c.id).sort()).toEqual([
      ...
      "flow-story-coverage", "render-coverage", "story-regression", "style-conformance",
      "token-conformance", "typography-conformance",
    ]);
    ...
    const conditional = new Set(["copy-conformance", "flow-steps", "flow-story-coverage", "story-regression", "style-conformance", "typography-conformance"]);
    expect(
